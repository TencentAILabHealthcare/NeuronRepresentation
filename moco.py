...
class MoCoTreeLSTM(nn.Module):
    def __init__(self, args, 
                dim=128, K=256, m=0.99, T=0.1, symmetric=True):
        ...
        # create the encoders
        self.backbone_q = model(x_size=len(args.input_features), 
                                  h_size=args.h_size, 
                                  num_classes=0, # fc unused
                                  fc=False,
                                  bn=args.bn,
                                  mode=args.child_mode)
        self.backbone_k = model(x_size=len(args.input_features), 
                                  h_size=args.h_size,
                                  num_classes=0, # fc unused
                                  fc=False,
                                  bn=args.bn,
        ...
        if args.projector_bn:
            self.projector_q = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), 
                                             nn.BatchNorm1d(dim_mlp, affine=False),
                                             nn.ReLU(), 
                                             nn.Linear(dim_mlp, dim))
            self.projector_k = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), 
                                             nn.BatchNorm1d(dim_mlp, affine=False),
                                             nn.ReLU(), 
                                             nn.Linear(dim_mlp, dim))
        else:
            self.projector_q = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), nn.Linear(dim_mlp, dim))
            self.projector_k = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), nn.Linear(dim_mlp, dim))
