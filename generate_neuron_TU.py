...
for dataset in ["seu_6_classes", "JM", "ACT"]:
    for split in ["train", "test"]:
        ...
        os.makedirs(
            f"GraphCL/unsupervised_TU/data/neuron/{dataset_name}/raw", exist_ok=True
        )
        adj = open(
            f"GraphCL/unsupervised_TU/data/neuron/{dataset_name}/raw/{dataset_name}_A.txt",
            "w",
        )
        graph_ids = open(
            f"GraphCL/unsupervised_TU/data/neuron/{dataset_name}/raw/{dataset_name}_graph_indicator.txt",
            "w",
        )
        graph_labels = open(
            f"GraphCL/unsupervised_TU/data/neuron/{dataset_name}/raw/{dataset_name}_graph_labels.txt",
            "w",
        )
        node_attributes = open(
            f"GraphCL/unsupervised_TU/data/neuron/{dataset_name}/raw/{dataset_name}_node_attributes.txt",
            "w",
        )
        ...
        for neuron_graph, label in zip(lines, targets):
            ...
            for ix, node_attr in enumerate(neuron_graph):
                ...
                if ix != 0:
                    adj.writelines(
                        [
                            f"{int(node_attr[0])+offset}, {int(node_attr[6]+offset)}\n",
...
adj = open(
    f"GraphCL/unsupervised_TU/data/neuron/{dataset_name}/raw/{dataset_name}_A.txt", "w"
)
graph_ids = open(
    f"GraphCL/unsupervised_TU/data/neuron/{dataset_name}/raw/{dataset_name}_graph_indicator.txt",
    "w",
