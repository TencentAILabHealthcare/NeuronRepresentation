...
from tree_dataset import NeuronTreeDataset, NeuronTreeDatasetTwoViews, get_collate_fn, LABEL_DICT
...
def test(net, memory_data_loader, test_data_loader, epoch, args, dataset_name):
    ...
    with torch.no_grad():
        ...
        for batch in test_data_loader:
            ...
            pred_labels = knn_predict(feature, feature_bank, feature_labels, classes, k, args.knn_t)
            total_num += feature.size(0)
            total_top1 += (pred_labels[:, 0] == batch.label.to(torch.device('cuda'))).float().sum().item()
    net.train()
...
if __name__ == '__main__':
    ...
    # Basic
    parser.add_argument('--work_dir', type=str, default='./work_dir', help='experiment root')
    parser.add_argument('--exp_name', type=str, default='debug', required=True, help='experiment name')
    parser.add_argument('--dataset', type=str, 
                        default='all_wo_others')
    parser.add_argument('--label_dict', type=str, 
                        default='all_wo_others')
    ...
    parser.add_argument('--epochs', default=100, type=int, help='number of epoch in training [default: 200]')
    parser.add_argument('--lr', '--learning-rate', default=0.06, type=float, metavar='LR', help='initial learning rate', dest='lr')
    parser.add_argument('--start_epoch', type=int, default=0, help='starting_epoch')
    parser.add_argument('--save_freq', type=int, default=5, help='Saving frequency [default: 5]')
    ...
    # Evaluation
    parser.add_argument('--val_freq', type=int, default=5, help='Val frequency [default: 5]')
    # Augmentation
    ...
    parser.add_argument('--drop_tree_prob', nargs="+", type=float, default=[0, 0, 0.005],
    ...
    # training dataset
    parser.add_argument('--train_split', type=str, default='full', help='[train | full]')
    parser.add_argument('--use_balanced_memory_data', action='store_true', default=False, 
    ...
    aug_switchs = [args.aug_scale_feats, args.aug_scale_coords, args.aug_rotate, 
                   args.aug_jitter_coords, args.aug_shift_coords, args.aug_flip, 
                   args.aug_mask_feats, args.aug_jitter_length, args.aug_elasticate]
    aug_fns = [ transforms.RandomScaleFeats(p=0.2),
                transforms.RandomScaleCoords(p=0.2) if not args.use_translation_feats \
    ...
    topo_aug_swtichs = [args.aug_drop_tree, args.aug_skip_parent_node, args.aug_swap_sibling_subtrees]
    topo_aug_fns = [
    ...
    topo_augs = [topo_aug_fns[i] for i in range(len(topo_aug_swtichs)) if topo_aug_swtichs[i] is True]
    if len(topo_augs) == 0:
    ...
    trainset = NeuronTreeDatasetTwoViews(
                        ...
                        label_dict=LABEL_DICT['all_wo_others'] if args.dataset== 'rebuttal' else LABEL_DICT[args.dataset],
    ...
    seu_memory_data =  NeuronTreeDataset(
                        phase='train', 
                        dataset='seu_6_classes' if not args.use_balanced_memory_data else 'seu_6_classes_balanced',
    ...
    for epoch in range(args.start_epoch+1, epochs+1):
            ...
            if args.knn:
                ...
                for j, eval_part in enumerate([model.backbone_q, model.encoder_q]):
                    for ix, (memory_loader, test_loader) in enumerate(zip(memory_loaders,test_loaders)):
                        test_acc = test(eval_part, memory_loader, test_loader, epoch, args, test_datasets[ix])
                        if test_acc > best_test_acc[j][ix]:
                            best_test_acc[j][ix], best_acc_epoch[j][ix] = test_acc, epoch
                        logger.info(
        ...
        if epoch % args.save_freq == 0:
            ...
            }, is_best=False, filename=f'{args.work_dir}/epoch_{epoch}.pth')
            logger.info(f"saving checkpoint at epoch {epoch} to {args.work_dir}/epoch_{epoch}.pth")