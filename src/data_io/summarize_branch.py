...
class Branch:
    ...
    def get_sequence(self):
        ...
        while nid < coord.shape[0]:
            ...
            if accu + cur_len >= step:
                coord_new.append(
                    coord_prev + (coord[nid, :] - coord_prev) / cur_len * (step - accu)
                )
                accu = 0
        ...
        # generate vector
        vec = np.array(
            [coord[i + 1, :] - coord[i, :] for i in range(coord.shape[0] - 1)]
        )
    ...
    def get_local_angle_between_plane(self, branch1, branch2):
        if (
            len(self.vertices) <= 1
            or len(branch1.vertices) <= 1
            or len(branch2.vertices) <= 1
        ):
            return np.arccos(0)
        v0 = self.vertices[1].coord - self.vertices[0].coord
        v1 = branch1.vertices[1].coord - branch1.vertices[0].coord
...
class NeuronSimplifier:
    ...
    def _compute_branch_feature(self, cur, parent, sibling):
        ...
        if parent:
            cur.labels["angle_remote_parent"] = [
                cur.get_remote_angle_between_branch(parent)
            ]
            cur.labels["angle_local_parent"] = [
                cur.get_local_angle_between_branch(parent)
            ]
        else:
            cur.labels["angle_remote_parent"] = [0]
            cur.labels["angle_local_parent"] = [0]
        if sibling:
            cur.labels["angle_remote_sibling"] = [
                cur.get_remote_angle_between_branch(sibling)
            ]
            cur.labels["angle_local_sibling"] = [
                cur.get_local_angle_between_branch(sibling)
            ]
        else:
            cur.labels["angle_remote_sibling"] = [0]
            cur.labels["angle_local_sibling"] = [0]
        if parent and sibling:
            cur.labels["angle_remote_plane"] = [
                cur.get_remote_angle_between_plane(parent, sibling)
            ]
            cur.labels["angle_local_plane"] = [
                cur.get_local_angle_between_plane(parent, sibling)
            ]
        else:
...
def script_test():
    N = Neuron()
    N.load_eswc(
        "/home/hanbo/workspace/external/morphvae/data/M1_exc_data/neurons/20190410_sample_10.swc"
    )
