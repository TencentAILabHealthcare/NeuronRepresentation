...
class Vertex:
    def __init__(self, vid, coordinate, ntype, parent=-1, radius=1):
...
class Neuron:
    ...
    def load_json(self, fname_json):
        ...
        for typename in ["axon", "dendrite"]:
            ...
            for node in data["neuron"][typename]:
                ...
                # connect with parent
                parent_vid = self.vertices[
                    dict_samplenumber_to_index[node["parentNumber"]]
                ].vid
                pidx = parent_vid - 1
                cidx = vid - 1
                self.vertices[pidx].add_child(cidx)
                self.vertices[cidx].parent = pidx
                self.vertices[cidx].len = np.linalg.norm(
                    self.vertices[pidx].coord - self.vertices[cidx].coord
                )
    ...
    def load_eswc(self, fname_eswc):
        ...
        for row in rows:
            ...
            for lid in range(len(items[7:])):
                label = items[lid + 7]
                label_name = (
                    col_names[lid + 7] if lid + 7 < len(col_names) else "label_%d" % lid
                )
                if label[0] == "#":  # comment
                ...
                try:
                    ...
                    self.warning(
                        "cannot recognize colume %d: %s, use default value -1."
                        % (lid + 7, row)
                    )
        ...
        if len(self.vertices) > 0:
            for label_name in self.vertices[0].labels:
                if label_name not in vtx.labels:
                    self.warning(
                        "new vertex does not have label value %s. skip" % label_name
                    )
                    return False
    ...
    def get_vtk_unstructured_data_grid(self, type_filter=None):
        ...
        if len(self.vertices) > 0 and len(self.vertices[0].labels) > 0:
            ...
            for label_name in self.vertices[0].labels:
                ...
                tmp_label.SetNumberOfComponents(
                    len(self.vertices[0].labels[label_name])
                )
        ...
        for ridx, rid in enumerate(self.roots):
            ...
            if flag_point_has_label:
                for label_name in labels_point:
                    labels_point[label_name].InsertNextTypedTuple(
                        vtx.labels[label_name]
                    )
    ...
    def save_svg(self, fname_svg, max_node=None):
        ...
        for vtx in vtx_list:
            ...
            plt.plot(
                [vtx.coord[1], vtx_p.coord[1]],
                [vtx.coord[0], vtx_p.coord[0]],
                "w",
                linewidth=0.5,
            )
        plt.axis("equal")
        ...
        for rid in self.roots:
            ...
            while len(queue) > 0:
                ...
                for child_pid in self.vertices[cur_pid].child:
                    tmp_dir = (
                        self.vertices[child_pid].coord - self.vertices[cur_pid].coord
                    )
    ...
    def normalize_neuron(
        self,
        flag_rotate=True,
        rotate_method="dendrite_pca_meanshift",
        ntype=None,
        dir_order="xyz",
    ):
        if ntype is None:
    ...
    def _override_type_DFS(self, vid, type_weight):
        ...
        while stack_all:
            ...
            for cid in self.vertices[pid].child:
                tmpt, tmpv = (
                    self.vertices[cid].type,
                    type_weight[self.vertices[cid].type],
                )
                if tmpv > maxv:
                    maxv = tmpv
                    maxt = tmpt
    ...
    def _angle_range_sample_searcher(
        self, init_dir, search_angle, ntype=None, constrain_dir=None
    ):
        thr_angle = math.cos(math.pi / 180.0 * search_angle)
        init_dir = np.array(init_dir)
        init_dir /= np.linalg.norm(init_dir)
        sample_dir = np.array(
            [vtx.coord for vtx in self.vertices if ntype is None or vtx.type in ntype]
        )
        sample_weight = np.array(
            [
                vtx.len * np.linalg.norm(vtx.coord)
                for vtx in self.vertices
                if ntype is None or vtx.type in ntype
            ]
        )
        if constrain_dir is not None:
            constrain_dir = np.array(constrain_dir)
            constrain_dir /= np.linalg.norm(constrain_dir)
            project_dir = constrain_dir[:, np.newaxis] * np.dot(
                constrain_dir.T, sample_dir.T
            )
            sample_dir -= project_dir.T
        angle = self._vector_to_vectors_cos(init_dir, sample_dir)
        return np.sum(sample_weight[angle > thr_angle])

    def _mean_shift_direction_searcher(
        self, init_dir, search_angle, ntype=None, constrain_dir=None
    ):
        thr_angle = math.cos(math.pi / 180.0 * search_angle)
        init_dir = np.array(init_dir)
    ...
    def _mean_shift_direction_searcher_v0(
        self, init_dir, search_angle, ntype=None, constrain_dir=None
    ):
        """
    ...
    def _mean_shift_direction(
        ...
        for i in range(max_iter):
            new_dir = self._mean_shift_direction_searcher(
                prev_dir, search_angle, ntype=ntype, constrain_dir=constrain_dir
            )
    ...
    def rotate_neuron_by_pca_meanshift(
        self, ntype=None, dir_order="zyx", attempt=0, search_angle_1=60
    ):
        ...
        for i in range(num_circle_samples):
            tmp_weight = self._angle_range_sample_searcher(
                tmp_dir, search_angle_1, ntype, dir_3
            )
            if tmp_weight > max_weight:
                max_weight = tmp_weight
                dir_1 = tmp_dir
            tmp_dir = r.apply(tmp_dir)
        dir_1 = self._mean_shift_direction(
            dir_1, ntype=ntype, constrain_dir=dir_3, search_angle=search_angle_1
        )
    ...
    def rotate_neuron_by_meanshift(
        self,
        ntype=None,
        dir_order="zyx",
        attempt=0,
        ...
        for vtx in self.vertices:
            ...
            if ntype is not None and vtx.type not in ntype:
                continue
            tmp_weight = self._angle_range_sample_searcher(
                vtx.coord, search_angle_1, ntype
            )
            if tmp_weight > max_weight:
                max_weight = tmp_weight
        ...
        dir_1 = self._mean_shift_direction(
            dir_1, ntype=ntype, search_angle=search_angle_1
        )
        ...
        for vtx in self.vertices:
            ...
            tmp_weight = self._angle_range_sample_searcher(
                tmp_dir, search_angle_2, ntype, dir_1
            )
            if tmp_weight > max_weight:
                max_weight = tmp_weight
                dir_2 = tmp_dir
        dir_2 = self._mean_shift_direction(
            dir_2, ntype=ntype, constrain_dir=dir_1, search_angle=search_angle_2
        )
        dir_3 = np.cross(dir_1, dir_2)
    ...
    def rotate_neuron_by_max_dir(self, ntype=None, dir_order="zyx", dir_method="tmap"):
        ...
        for order in range(3):
            count = np.sum(
                [vtx.coord[order] for vtx in self.vertices if vtx.type in ntype]
            )
            if count < 0:
                for vidx, vtx in enumerate(self.vertices):
                    self.vertices[vidx].coord[order] *= -1
    ...
    def rotate_neuron_by_axon(self):
        ...
        for idx in range(len(self.roots_dir)):
            tmp_dir = (
                np.dot(self.roots_dir[idx], vector_x),
    ...
    def remove_fragements(self, flag_del_frag=False, flag_check_root_type=False):
        ...
        if flag_check_root_type:
            if self.vertices[max_root_idx].type != 1:
                self.error(
                    "Warning, node type of maximum root is not 1. Found %d"
    ...
    def _search_axons_subtree(self):
        ...
        for idx, vtx in enumerate(self.vertices):
            flag_is_root = False
            if vtx.type == 2:
                if vtx.parent < 0 or self.vertices[vtx.parent].type != 2:
                    flag_is_root = True
                elif (
                    self.vertices[vtx.parent].labels["label_0"][0]
                    != vtx.labels["label_0"][0]
                ):
                    flag_is_root = True
                    list_parent_labels.append(
                        self.vertices[vtx.parent].labels["label_0"][0]
                    )
            if flag_is_root:
                if vtx.labels["label_0"][0] in list_axon_roots:
                    print(
                        "Warning: identify label %d record for multiple sub-tree, check the data!"
                        % vtx.labels["label_0"][0]
                    )
                    continue
    ...
    def _remove_middle_axons(
        self, list_axon_roots, thr_minimum_num_nodes=15, thr_minimum_num_tips=13
    ):
        list_lid_to_del = []
        for lid in list_axon_roots.keys():
            if len(list_axon_roots[lid]["list_vtx"]) < thr_minimum_num_nodes:
                ...
                list_axon_roots[lid]["connect_to_others"]
                and list_axon_roots[lid]["tip"] < thr_minimum_num_tips
            ):
                list_lid_to_del.append(lid)
        for lid in list_lid_to_del:
            del list_axon_roots[lid]

    def _mean_shift_vtx_center_and_radius(
        self, list_vtx, interval_scale=1, max_shift_times=10, converge_shift=0.1
    ):
        ...
        for i in range(max_shift_times):
            ...
            center_new = np.sum(coord_new * weight_new[:, np.newaxis], axis=0) / np.sum(
                weight_new
            )
        ...
        return {
            "radius": radius,
            "center": center_pre,
            "radius_coverage": percent_coverage,
        }
    ...
    def get_axon_subtree_by_label(self):
        ...
        for lid in list_axon_roots:
            res = self._mean_shift_vtx_center_and_radius(
                list_axon_roots[lid]["list_vtx"]
            )
...
class NeuronBundle:
    ...
    def get_vtk_unstructured_data_grid(self, type_filter=None):
        ...
        for neuron in self.neurons:
            ug.AddInputData(
                neuron.get_vtk_unstructured_data_grid(type_filter=type_filter)
            )
        ug.Update()
        return ug.GetOutput()
