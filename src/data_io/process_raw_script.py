...
def filter_axon_and_check(folder_in, folder_out):
    ...
    for fname in tqdm(filelist):
        ...
        neuron.load_eswc(fname)
        if len(neuron.roots) != 1:
            print(fname,'has %d roots! Will remove fragements and keep only 1 root.'%len(neuron.roots))
            neuron.remove_fragements(flag_del_frag=True)
        neuron.override_type()
...
def split_sample_10fold_cv_and_merge(folder_data):
    ...
    sample_dict = defaultdict(list) #key is group name, value is next availabel fold idx
    flist_csv = [os.path.join(folder_info,'ACT_info_swc.csv'),
    ...
    for fname_csv in flist_csv:
        ...
        for rid, row in df.iterrows():
            group = '%s_layer%s'%(str(row['structure_parent__acronym']).replace(',',''),
