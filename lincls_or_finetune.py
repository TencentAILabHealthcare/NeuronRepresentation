...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', default=1e-4, type=float, help='init learning rate [default: 1e-4]')
    parser.add_argument('--start_epoch', type=int, default=0, help='starting_epoch')
    parser.add_argument('--save_freq', type=int, default=10, help='Saving frequency [default: 1]')
    ...
    # linear evaluation
    parser.add_argument('--mode', type=str, default='finetune', help='random_init | frozon | finetune')
    ...
    model = model(len(args.input_features), h_size, len(LABEL_DICT[args.dataset]), mode=args.child_mode,
    ...
    for epoch in range(args.start_epoch+1, epochs+1):
        ...
        for step, batch in enumerate(train_loader):
            ...
            acc = float(torch.sum(torch.eq(batch.label.cuda(), pred))) / len(batch.label)
        ...
        if epoch % args.val_freq == 0:
            ...
            if epoch % args.save_freq == 0 or is_best:
                ...
                }, is_best=is_best, filename=f'{args.work_dir}/epoch_{epoch}.pth')
