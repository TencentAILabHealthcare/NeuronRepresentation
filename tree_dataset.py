...
NeuronBatchTwoViews = namedtuple('NeuronBatchTwoViews', 
                                 ['view1', 'view2'])

all_wt_others = {'VPM': 0, 'others': 1, 'Isocortex_layer23': 2, 
                 'Isocortex_layer4': 3, 'PRE': 4, 'SUB': 5, 
                 'CP': 6, 'VPL': 7, 'Isocortex_layer6': 8, 
                 'MG': 9, 'Isocortex_layer5': 10}
...
SEU_6_classes = {'CP': 0, 'Isocortex_layer23': 1, 'Isocortex_layer4': 2, 
               'Isocortex_layer5': 3, 'Isocortex_layer6': 4, 'VPM': 5}

JM_classes = {'Isocortex_layer23': 0, 'Isocortex_layer5':1, 
             'Isocortex_layer6':2, 'VPM':3}

ACT_classes = {'Isocortex_layer23': 0, 'Isocortex_layer4':1, 
               'Isocortex_layer5':2, 'Isocortex_layer6':3, }

LABEL_DICT = {'all_wt_others':all_wt_others, 'all_wo_others':all_wo_others,
              'seu_6_classes': SEU_6_classes, 'JM': JM_classes,
              'ACT': ACT_classes}
...
def get_collate_fn(device, use_two_views=False):
    ...
    def collate_fn_two_views(batch):
        ...
        cnt1, cnt2 = 0, 0
        for b in batch:
            trees1.append(b[0]); trees2.append(b[2])
            cnt1 += b[1];  cnt2 += b[3]
            offsets1.append(cnt1); offsets2.append(cnt2)
        offsets1.pop(-1); offsets2.pop(-1)
...
class NeuronTreeDataset(object):
    def __init__(self,
        ...
        print(self.dataset, phase, 'distribution:', np.unique(self.targets, return_counts=True))
    ...
    def load_filecache(self):
        if self.dataset != 'rebuttal':
            ...
            for dataset in ['seu_6_classes', 'JM', 'ACT']:
                saved_cache = f'datasets/neuron_morpho/processed_datasets_bk2/{dataset}_train.pkl'
                print('loading from', saved_cache)
                with open(saved_cache, 'rb') as f:
                    saved_cache = pickle.load(f)
    ...
    def __getitem__(self, idx):
        if self.attribute_transformations is None \
...
def process_diff_datasets():
    ...
    seu_classes = ['CP', 'Isocortex_layer23', 'Isocortex_layer4', 'Isocortex_layer5',
                'Isocortex_layer6', 'VPM']
    ...
    JMclasses_label_dict = {'Isocortex_layer23': 1, 'Isocortex_layer5':3, 
                            'Isocortex_layer6':4, 'VPM':5}
    ACT_classes = ['Isocortex_layer23', 'Isocortex_layer5', 'Isocortex_layer6', 'Isocortex_layer4']
    ACT_classes_label_dict = {'Isocortex_layer23': 1, 'Isocortex_layer5':3, 
                            'Isocortex_layer6':4, 'Isocortex_layer4':2}
    JM_test = []
    ...
    for label in classes:
        if label == 'others':
            ...
            for fine_label in subclasses:
                ...
                full_wt_others += [f'{root}/{label}/{fine_label}/{x}' for x in candidate_folds]
        else:
