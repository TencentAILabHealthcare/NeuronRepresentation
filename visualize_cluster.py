...
def plot_clusters_img(fname_output, img_list, img_size=1000):
    ...
    img = np.zeros((padding*2+step*len(img_list), padding*2+step*len(img_list[0]), 3))
...
def plot_tsne_img(fname_output, img_list, r_list, c_list, cache=None, task_name='task'):
    ...
    for img_info, r_center, c_center in tqdm(zip(img_list, r_list, c_list),total=len(img_list),
                                             desc=task_name):
        if cache is None:
            sub_img = cv2.imread(img_info[0])
        else:
...
def script_plot_tsne_graphcl():
    ...
    scale_coord = 100
    sample_frac = 1
    file_cache = pkl.load(open('/apdcephfs/private_jiaweiyyang/project/neuron/NeuronRepresentation/datasets/neuron_morpho/processed_datasets_bk2/all_wo_others_full.pkl', 'rb'))
    ...
    for view in views:
        ...
        for attribute in attributes:
            ...
            for _, row in df.iterrows():
                ...
                fname_img = os.path.join(root_path,
                                        # row[col_dataset],
                                        _ds,
                                        # 'input_neuron',
                ...
                elif attribute in ['class', 'height', 'n_nodes', 'avg_length', 'total_length', 'avg_contraction', 'std_contraction']:
            ...
            plot_tsne_img(fname_output+'_%s.png'%view, img_list,r_list, c_list, cache, 
...
def script_plot_tsne_vae():
    ...
    vae_filelist = open('rebuttal/train_seu_6class_xyzlcV24_model_best.pt_all_filelist.txt').readlines()
...
def script_plot_tsne2():
    ...
    attributes = ['height', 'n_nodes', 'avg_length', 'total_length', 'avg_contraction', 'std_contraction']
    for attribute in attributes:
        ori_df[attribute] = (ori_df[attribute]-ori_df[attribute].min())/(ori_df[attribute].max()-ori_df[attribute].min())
    attributes.insert(0, 'class')
    attributes.insert(1, 'dataset')
    ...
    for view in views:
        ...
        for attribute in attributes:
            ...
            for _, row in df.iterrows():
                ...
                #sub_img.append((255-img[350:650,350:650,:],None))
                if attribute == 'dataset':
                    color = [1-_ for _ in cmap(dataset_dict[row[attribute]])[:3][::-1]]
                elif attribute in ['class', 'K=20','height', 'n_nodes', 'avg_length', 'total_length', 'avg_contraction', 'std_contraction']:
